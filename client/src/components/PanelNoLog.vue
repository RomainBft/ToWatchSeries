...
export default {
  ...
  async mounted () {
    ...
    const itemOptions = item2.querySelector('.item-options')
    const itemField = item2.getElementsByClassName('anim-field')
    const checkbox = item2.querySelector('input[type=checkbox')
    let tl = new TimelineMax({repeat: -1, repeatDelay: 2})
    tl.set('.item-options', {opacity: 0, y: 0, height: 0, marginTop: 0})
      .to(item2, {className: 'panel-item show', duration: 0.25})
      .fromTo(itemOptions, {opacity: 0, y: 0, height: 0, marginTop: 0}, {opacity: 1, y: -10, height: "auto", marginTop: 15, duration: 0.25})
      .fromTo(itemField, {opacity: 0, y: -20}, {opacity: 1, y: 0, duration: 0.5, stagger: 0.05})
      .to(checkbox, {checked: true, duration: 1, onComplete: () => { this.seriestats[0].see = true } })
      .to(item2, {className: 'panel-item see', duration: 0.25, delay: 1})
      .fromTo(itemField, {opacity: 1, y: 0}, {opacity: 0, y: -20, duration: 0.25})
      .fromTo(itemOptions, {opacity: 1, y: -10, height: "auto", marginTop: 21}, {opacity: 0, y: 0, height: 0, marginTop: 0, duration: 0.25})
      .to(item2, {className: 'panel-item see show', duration: 0.25, delay: 2})
      .fromTo(itemOptions, {opacity: 0, y: 0, height: 0, marginTop: 0}, {opacity: 1, y: -10, height: "auto", marginTop: 15, duration: 0.25})
      .fromTo(itemField, {opacity: 0, y: -20}, {opacity: 1, y: 0, duration: 0.5, stagger: 0.05})
      .to(checkbox, {checked: false, duration: 1, onComplete: () => { this.seriestats[0].see = false } })
      .to(item2, {className: 'panel-item', duration: 0.25, delay: 1})
      .fromTo(itemField, {opacity: 1, y: 0}, {opacity: 0, y: -20, duration: 0.25})
      .fromTo(itemOptions, {opacity: 1, y: -10, height: "auto", marginTop: 15}, {opacity: 0, y: 0, height: 0, marginTop: 0, duration: 0.25})
